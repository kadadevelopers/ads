package com.kadadevelopers.ads.banner;
    ...
    public void loadBannerAd() {
        if (adStatus.equals(AD_STATUS_ON) && placementStatus != 0) {
            switch (adNetwork) {
                ...
                case APPODEAL:
                    Appodeal.setBannerViewId(R.id.appodeal_banner_view_container);
                    Appodeal.show(activity, Appodeal.BANNER_VIEW);
                    Appodeal.setBannerCallbacks(new BannerCallbacks() {
                        @Override
                        public void onBannerLoaded(int height, boolean isPrecache) {
                            // Called when banner is loaded
                            activity.findViewById(R.id.appodeal_banner_view_container).setVisibility(View.VISIBLE);
                            Log.d(TAG, "appodeal banner ad loaded");
                        }

                        @Override
                        public void onBannerFailedToLoad() {
                            // Called when banner failed to load
                            activity.findViewById(R.id.appodeal_banner_view_container).setVisibility(View.GONE);
                            loadBackupBannerAd();
                            Log.d(TAG, "failed to load appodeal banner ad");
                        }
